...
namespace RubySharp {
	...
	public class RubyState : IDisposable {
		...
		private ObjectTranslator _translator = new ();

		// private Dictionary< int, GCHandle > gcHandleDictionary = new Dictionary< int, GCHandle > ();
		private readonly Map< IntPtr, int > _typedReferenceMap = new ();
		private readonly Map< object, R_VAL > _dataValueMap = new ();

		// 记录绑定类传递到ruby中mrb_data_type结构体, 包括反射绑定和生成代码绑定
		private readonly Dictionary< Type, mrb_data_type > _registedDataTypeDictionary = new ();
		private readonly Dictionary< Type, IntPtr > _registedDataTypePtrDictionary = new ();
   ...
   //          
		...
		public RegistedTypeInfo GetRegistedTypeInfo( Type t ) {
			foreach ( var kv in _registedTypeInfoDictionary ) {
				if ( kv.Key == t ) {
					return kv.Value;
				}
			}
		...
		public void AddRegistedTypeInfo< T >( IntPtr @class, mrb_data_type dataType, IntPtr dataTypePtr ) {
			_registedTypeInfoDictionary.Add( typeof( T ),
		...
		public IntPtr PushRegistedCSharpObject( object obj ) {
			...
			int index;
			if ( !_translator.Getudata( obj, out index ) ) {
				index = _translator.AddObject( obj );
				...
				_typedReferenceMap.Add( indexPtr, index );
			...
			return _typedReferenceMap.Reverse[ index ];
		}
		...
		public IntPtr PushCSharpObject( object obj ) {
			...
			if ( !_translator.Getudata( obj, out var index ) ) {
				index = _translator.AddObject( obj );
		...
		public void AffirmDataTypeStruct< T >( out mrb_data_type data_type_t, out IntPtr data_type_ptr ) {
			...
			// Use exist
			if ( _registedDataTypeDictionary.ContainsKey( type ) ) {
				data_type_t = _registedDataTypeDictionary[ type ];
				
				if ( _registedDataTypePtrDictionary.ContainsKey( type ) ) {
					data_type_ptr = _registedDataTypePtrDictionary[ type ];
				}
				return;
			}
			...
			_registedDataTypeDictionary.Add( type, data_type_t );
			_registedDataTypePtrDictionary.Add( type, data_type_ptr );
		}
		...
		public bool HasC2RInstanceMap( object obj ) {
			return _dataValueMap.Forward.Contains( obj );
		}
		...
		public bool HasR2CInstanceMap( R_VAL val ) {
			return _dataValueMap.Reverse.Contains( val );
		}
		...
		public void DFree( IntPtr mrb, IntPtr data ) {
			...
			if ( obj != null && _translator.Getudata( obj, out index ) ) {
				...
				_translator.RemoveObject( index );
				_typedReferenceMap.Remove( data );
		...
		public static object ValueToRefObject( RubyState state, R_VAL value, IntPtr data_type_ptr ) {
			...
			if ( state.HasR2CInstanceMap( value ) ) {
				...
				#endif
				return state._dataValueMap.Reverse[ value ];
			}
			
			IntPtr ptr = RubyDLL.mrb_data_get_ptr( state, value, data_type_ptr );
			// return ( ( GCHandle )ptr ).Target;
			return state._translator.GetObject( state._typedReferenceMap.Forward[ ptr ] );
		}
        ...
        void IDisposable.Dispose() {
			...
			foreach ( var kv in _registedDataTypePtrDictionary ) {
				Marshal.FreeHGlobal( kv.Value );
			}
			
			// gcHandleDictionary.Clear ();
			_typedReferenceMap.Clear();
			_dataValueMap.Clear();
			_registedDataTypeDictionary.Clear();
			_registedDataTypePtrDictionary.Clear();
